            ...
            <div className="lg:py-2 lg:pl-16">
              ...
              <div className="mx-auto mt-10 sm:mt-12">
                <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-2">
                  ...
                  <a
                    href={
                      process.env.NEXT_PUBLIC_CALENDLY_URL_BOOK_A_DEMO || '#'
                    }
...
const VideoDemoSection: React.FC<{ localeNamespace: string }> = ({
  localeNamespace,
}) => {
  const { t: translate } = useTranslation(localeNamespace)
...
const FeaturesSection: React.FC<{ localeNamespace: string }> = ({
  localeNamespace,
}) => {
...
const CTA: React.FC<{ localeNamespace: string }> = ({ localeNamespace }) => {
  const { t: translate } = useTranslation(localeNamespace)
...
const Home: NextPageWithLayout<{
  ...
  return (
    <>
      ...
      <main>
        <HeroSection localeNamespace={localeNamespace} />
        <VideoDemoSection localeNamespace={localeNamespace} />
        <FeaturesSection localeNamespace={localeNamespace} />
        <BlockchainAvailable />
        <TestimonialSection n={10} slideInterval={3} />
        {/*<BlogSection />*/}
        <CTA localeNamespace={localeNamespace} />
      </main>
    </>
  )
}
...
export const getStaticPaths = async () => {
  const experiment =
    getGoogleOptimizeCurrentExperiment() as GoogleOptimizeExperiment

  return {
    paths: experiment.variants.map((variant) => ({
...
export const getStaticProps: GetStaticProps = async ({ params, locale }) => {
  const experiment =
    getGoogleOptimizeCurrentExperiment() as GoogleOptimizeExperiment
  const [, variantId] = (params?.variant as string | undefined)?.split('.') || [
    undefined,
    '',
  ]
  ...
  const localeNamespace = formatExperimentLocale(
    'homepage',
    experiment.id,
    variant.id
  )
